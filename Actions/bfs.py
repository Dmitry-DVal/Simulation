...
class BFS:
    ...
    def is_in_range(self, peaks: tuple[int, int]) -> bool:
        '''Проверка входят ли точки в границы карты.'''
    ...
    def bfs(self, creature: 'Creature'):
        """Ищет близжайщий маршрут до существа"""
        first_peak = creature.coordinate
        visited_peak = set()
        ...
        while queue:
            ...
            for nx, ny in self.get_neighbors((x, y)):
                ...
                if isinstance(self.Simulation.Map.map[(nx, ny)], creature.goal):
                    if level + 1 <= creature.speed:  # 1ый вариант, существо успевает дойти до цели
                        return creature.make_move(creature, first_peak, (nx, ny), level, counter,
                                                  path, self.Simulation.Map.map, self.Simulation.living_creatures,
                        ...
                        return creature.move_closer_to_goal(possible_final_coordinate, nx, ny, path, first_peak,
                                                            creature, self.Simulation.Map.map)
                else:
                    queue.append(((nx, ny), level + 1, path + [(nx, ny)]))
        print(f'Существо не видит цель и остается на месте {first_peak}')
        return []
